/* ==========================================================================
   СЕРВЕР «КИБЕРНИКИТКА» v3.2 — REST API (ПОЛНАЯ СОХРАНЯЕМОСТЬ И ФОРУМ 2000-х)
   ========================================================================== */
...
let queryTursoCloud;
try {
  const tursoModule = require('./db/turso.js');
...
let backupData = { disks: [], guestbook: [] };
...
function sendJson(res, statusCode, data) {
  res.writeHead(statusCode, {
    ...
    'Access-Control-Allow-Headers': 'Content-Type, X-Cyber-Token, Authorization'
  });
...
async function getUserByToken(req) {
  ...
  if (memorySessions.has(token)) return memorySessions.get(token);
    ...
    try {
      ...
      if (sessRows && sessRows.length > 0) {
        ...
        if (userRows && userRows.length > 0) {
          const u = userRows[0];
          const userObj = {
            id: u.id, username: u.username, role: u.role, display_name: u.display_name || u.username,
            icq_uin: u.icq_uin || '', avatar_url: u.avatar_url || 'img/photo-nikitka.png', city: u.city || 'Москва',
            qip_status: u.qip_status || '🟢 В сети (Online)', text_quote: u.text_quote || 'Диалап рвется, но мы держимся!',
            pc_specs: u.pc_specs || 'Core 2 Duo E6600 | 2GB DDR2 | GeForce 8800GT', views_count: u.views_count || 184
          };
...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname === '/api/auth/register' && method === 'POST') {
    try {
      ...
      if (!username || username.length < 3) return sendJson(res, 400, { success: false, error: { message: 'Никнейм от 3 символов!' } });
      if (!password || password.length < 4) return sendJson(res, 400, { success: false, error: { message: 'Пароль от 4 символов!' } });
  ...
  if (pathname === '/api/auth/login' && method === 'POST') {
    try {
      ...
      if (!username || !password) return sendJson(res, 400, { success: false, error: { message: 'Укажите логин и пароль!' } });
      ...
      if (!userObj) {
        userObj = {
          ...
          avatar_url: 'img/avatar-cs.png', qip_status: '🟢 В сети (Online)', text_quote: 'Диалап рвется!', pc_specs: 'Core 2 Duo E6600 | 2GB DDR2 | GeForce 8800GT', views_count: 184
        };
  ...
  if (pathname === '/api/user/profile' && (method === 'PUT' || method === 'POST')) {
    const user = await getUserByToken(req);
  ...
  // 5. POST /api/user/wall
  if (pathname === '/api/user/wall' && method === 'POST') {
    const user = await getUserByToken(req);
    const body = await parseJsonBody(req);
  ...
  // 6. GET /api/user/cabinet
  if (pathname === '/api/user/cabinet' && method === 'GET') {
    const user = await getUserByToken(req);
    if (!user) return sendJson(res, 401, { success: false, error: { message: 'Требуется авторизация!' } });
    ...
    const wallComments = memoryWallComments.get(user.username) || [
      { id: 'w-init-1', author: 'nikitka_master', message: 'Привет! Оставил первый автограф на твоей стенке 2007 года!', created_at: new Date().toISOString() }
    ];
    ...
    return sendJson(res, 200, {
      success: true,
      data: { profile: user, rating: autoRating, wall_comments: wallComments, disks: userDisks, repair_requests: userRequests, forum_topics: userTopics, forum_posts: userPosts }
    });
  }

  // 7. GET /api/forum/categories
  if (pathname === '/api/forum/categories' && method === 'GET') {
    let cats = [];
    ...
    return sendJson(res, 200, {
      ...
      data: {
        categories: cats.length ? cats : [
          { id: 'cat-01', title: 'Железо и Софт 2000-х', description: 'Обсуждение Socket LGA775 и Windows XP', order_index: 1 },
  ...
  // 8. POST /api/forum/topics
  if (pathname === '/api/forum/topics' && method === 'POST') {
    try {
      const body = await parseJsonBody(req);
      const user = await getUserByToken(req);
      ...
      if (!title || !content) return sendJson(res, 400, { success: false, error: { message: 'Название темы и текст обязательны!' } });
      ...
      if (queryTursoCloud) {
        try {
          await queryTursoCloud('INSERT INTO forum_topics (id, category_id, title, author, is_pinned, is_closed, views_count, replies_count, created_at, updated_at) VALUES (?, ?, ?, ?, 0, 0, 1, 0, ?, ?)', [topic_id, category_id, title, author, dateStr, dateStr]);
      ...
      return sendJson(res, 201, { success: true, data: { topic_id, post_id, category_id, title, author, content, created_at: dateStr } });
    } catch (err) {
      return sendJson(res, 500, { success: false, error: { message: err.message } });
    }
  }

  // 9. POST /api/forum/posts
  if (pathname === '/api/forum/posts' && method === 'POST') {
    try {
      ...
      if (!content) return sendJson(res, 400, { success: false, error: { message: 'Текст не может быть пустым!' } });
      ...
      return sendJson(res, 201, { success: true, data: { id: post_id, topic_id, author, content, pc_specs, created_at: dateStr } });
    } catch (err) {
      return sendJson(res, 500, { success: false, error: { message: err.message } });
    }
  }

  // СТАБИЛЬНЫЕ РУЧКИ КОНТЕНТА
  if (pathname === '/api/disks' && method === 'GET') {
    let list = backupData.disks || [];
    return sendJson(res, 200, { success: true, data: list, fallback: true });
  }

  // РАЗДАЧА СТАТИКИ
  let filePath = path.join(PUBLIC_DIR, pathname === '/' ? 'index.html' : pathname);
  if (!filePath.startsWith(PUBLIC_DIR)) filePath = path.join(PUBLIC_DIR, '404.html');
...
server.listen(PORT, () => {
  console.log(`\n=================================================================`);
  console.log(` 🚀 СЕРВЕР «КИБЕРНИКИТКА» v3.2 (FULL PERSISTENCE) ЗАПУЩЕН!`);
